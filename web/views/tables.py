...
class TableSettingsConfig(StarkConfig):
    ...
    def get_urls(self):
        ...
        urlpatterns = [
            ...
            re_path(r'^settings/$', self.wrapper(self.table_setting), name='table_setting'),
            re_path(r'^(?P<pk>\d+)/setting_edit/$', self.wrapper(self.edit_setting), name='edit_setting'),
    ...
    def table_setting(self, request):
        '''
        表单设置页面
        :param request:
        :return:
    ...
    def edit_setting(self, request, *args, **kwargs):
        ...
        if request.is_ajax():
            try:
                ...
                old_fields = models.SettingToField.objects.filter(setting=setting_obj).all().distinct().order_by(
                    'order')
                ...
                for field in new_fields_ids:
                    ...
                    if field not in old_fields_ids:
                        obj = models.SettingToField.objects.create(setting=setting_obj, fields_id=int(field),
                                                                   order=index)
                        temp.append(obj)
                        temp.append(obj.pk)
